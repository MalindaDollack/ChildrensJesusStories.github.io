...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (staplesCard) {
    ...
    if (mainImage) { mainImage.src = 'staples-current-front.jpg'; mainImage.alt = 'Current Staples printed Sarah the Baby Sheep soft cover book'; }
    ...
    if (!staplesCard.querySelector('.staples-photo-gallery')) {
      ...
      const photos = [['staples-current-front.jpg','Current Staples Printed Front Cover'],['staples-back-cover.jpg','Back Cover'],['staples-pages-1-2.jpg','Pages 1 + 2'],['staples-pages-3-4.jpg','Pages 3 + 4'],['sarah-future-printing-cover.jpg','New Cover — Future Printings']];
  ...
  function makeAvailable(button) { if (!button) return; button.disabled = false; button.classList.remove('disabled'); button.textContent = 'SOFTCOVER AVAILABLE NOW — C$22.27'; button.style.background = '#176b27'; button.style.cursor = 'pointer'; button.addEventListener('click', () => { const target = document.getElementById('staples-book-card'); if (target) target.scrollIntoView({behavior:'smooth', block:'center'}); }); }
  ...
  if (standardEbookCard) {
    const img = standardEbookCard.querySelector('img'); if (img) { img.src = standardCoverFile; img.alt = 'Sarah the Baby Sheep Standard PDF E-Book Edition cover'; }
  }
  ...
  if (flipEbookCard) { const flipOrder = flipEbookCard.querySelector('.store-order a'); if (flipOrder) flipOrder.href = 'mailto:dollackj316@gmail.com?subject=Order%20inquiry%3A%20Flip%20Book%20E-Book%20Edition%20Link&body=Hello%20Malinda%2C%0A%0AI%20would%20like%20-%20The%20Link%20to%20the%20Flip%20Book%20E-Book%20Edition%0A%0AMy%20name%3A%0AMy%20e-mail%20address%3A%0A%0APlease%20send%20me%20the%20e-Transfer%20instructions.%0A'; }
  ...
  if (sarahPreview) {
    ...
    if (buy) { const purchase = buy.querySelector('a'); if (purchase) purchase.textContent = 'Buy Now !'; }
  }
  ...
  if (orderingHeading) { let next = orderingHeading.nextElementSibling; while (next && !/^H[1-3]$/.test(next.tagName)) { if (/softcover:\s*coming soon|ordering will open soon/i.test(next.textContent || '')) { next.textContent = 'E-book: C$7.00 · Softcover: C$22.27. Interac e-Transfer is accepted for paid products. For physical products, e-mail Malinda for exact shipping costs before sending payment.'; break; } next = next.nextElementSibling; } }
});
