...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (staplesCard) {
    ...
    if (mainImage) {
      mainImage.src = 'sarah-future-printing-cover.jpg';
      mainImage.alt = 'Sarah the Baby Sheep Photo Book - Malinda Dollack cover';
    }
    ...
    if (!staplesCard.querySelector('.staples-photo-gallery')) {
      ...
      const photos = [
        ['sarah-future-printing-cover.jpg','Photo Book Cover'],
  ...
  const photoBookMail = 'mailto:dollackj316@gmail.com?subject=Order%20inquiry%3A%20Staples%20Photo%20Book%20Edition&body=Hello%20Malinda%2C%0A%0AI%20would%20like%20to%20order%20the%20Photo%20Book%20Edition.%0A%0AMy%20name%3A%0AMy%20mailing%20address%3A%0A%0APlease%20send%20me%20the%20e-Transfer%20instructions%20and%20shipping%20information.%0A';
  function makePhotoBookAvailable(button) {
    ...
    button.onclick = () => { window.location.href = photoBookMail; };
  }
  ...
  if (flipEbookCard) {
    ...
    if (!flipEbookCard.querySelector('.flip-preview-link')) {
      const preview = document.createElement('a');
      preview.className = 'flip-preview-link';
      preview.href = 'flip-book-preview.html';
      preview.target = '_blank';
      preview.rel = 'noopener';
      preview.textContent = 'View 5-Page Flip Book Preview';
      preview.style.cssText = 'display:block;margin:12px 0 4px;padding:12px 14px;border-radius:999px;background:#176b27;color:#fff;text-decoration:none;text-align:center;font-weight:900;box-shadow:0 5px 12px rgba(23,107,39,.18)';
      const order = flipEbookCard.querySelector('.store-order');
      if (order) order.insertAdjacentElement('beforebegin', preview);
      else flipEbookCard.appendChild(preview);
    }
  }
  ...
  if (sarahPreview) {
    ...
    if (buy) {
      ...
      makePhotoBookAvailable(buy.querySelector('button'));
    }
  }
